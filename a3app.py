...
def hsl2hsv(color):
    """
    Returns the hsv equivalent of an hsl value

    Parameter color: the color to convert
    Precondition: color is an HSL object
    """
    hue = color[0]
...
class SliderField(BoxLayout):
    ...
    def update_on_slide(self, instance, value):
        ...
        maxval = self.max_val/self.factor
        if self.value == value:  # Hack because of poor Kivy design
            self.value = value+1 if value < maxval else value-1
        self.value = value
        if self.field:
...
class ColorWidget(BoxLayout):
    ...
    def syncInput(self, instance, value):
        ...
        if instance == self.rgbPanel:
            ...
            rgb  = introcs.RGB(self.rgb.red,self.rgb.green,self.rgb.blue)
            a3.cmyk_to_rgb(self.cmyk,rgb)
